...
author: "Group 3"
date: "`r format(as.Date('2026-09-25'))`"
output:
  html_document: 
  toc: true 
...
1. **Nathan Rhodes** — 20560116
    + [BIOL343_CC](https://github.com/28Nate28/BIOL343_CC)
1. 
...
------|---------|------
population_type | Distinguishes wether a flower is Asexual or Sexual | Categorical (A / S)
ovary_width | Measure of ovary width for each flower | milimeters
flower_number| The number assigned to individual flower |  ID
...
```{r import-clean, include=F}
cc1_data <- read.csv("A1_Data_Cleaned.csv")

cc1_data$ovary_area <- cc1_data$ovary_width * cc1_data$ovary_height
```
...
Ovary Area is measured in $\text{mm}^2$
...
area_means <- aggregate(cc1_data$ovary_area, list(cc1_data$pop_type), mean)
...
```{r area-table, echo=F}
library(knitr)
kable(area_means, caption = "Summary of Ovary Means")
```
...
ggplot(data = cc1_data, aes(x = ovary_area, fill = pop_type)) +
  geom_histogram(binwidth = 1) +
  facet_wrap(~pop_type, ncol = 2) +
  labs(
    title = "Ovary Area by Population Type",
    x = "Population Type",
    y = "Area (mm^2)"
  ) +
  theme_classic()
...
![_Solanum pimpinellifolium_](./images/s_pimp.jpeg)

Source: [Google Image](https://www.google.com/imgres?q=solanum%20pimpinellifolium&imgurl=https%3A%2F%2Fwww.magicgardenseeds.com%2Fmedia%2Fimage%2Fproduct%2F1527%2Flg%2Fwildtomate-rote-murmel-solanum-pimpinellifolium-bio-saatgut.jpg&imgrefurl=https%3A%2F%2Fwww.magicgardenseeds.com%2FWild-Tomato-Rote-Murmel-Solanum-pimpinellifolium-Organic-seeds%3Fsrsltid%3DAU7gw4UfCTWW8xeRG0o5RyYBXkXlODLl6pSFvo3y39kpr0QHX0RALg0g&docid=N4V7kO_Jtp0UQM&tbnid=2shC-zTXTQ3g6M&vet=12ahUKEwi_w4OA44eXAxW_qSsGHVgdHekQnPAOegUInwEQAA..i&w=1200&h=1200&hcb=2&ved=2ahUKEwi_w4OA44eXAxW_qSsGHVgdHekQnPAOegUInwEQAA)
